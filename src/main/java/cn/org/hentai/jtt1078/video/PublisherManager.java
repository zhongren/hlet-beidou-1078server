...
import cn.org.hentai.jtt1078.server.RtpMessage;
...
public final class PublisherManager
{
    ...
    public Map<Long, Publisher> getPublishers() {
        return publishers;
    }

    public void setPublishers(Map<Long, Publisher> publishers) {
    ...
    // 先申请一个推送通道
    public long request(String rtmpURL,String sim,int lcn) throws Exception
    {
        ...
        synchronized (lock)
        {
            Publisher publisher = new Publisher(id,sim,lcn);
    ...
    // 转发H264数据到FIFO中
    public void publish(long id, RtpMessage message) throws Exception
    {
        ...
        if (publisher.publish(message.getBody())==false)
        {
            publisher.close();
            synchronized (lock)
            {
                publishers.remove(publisher.publisherId);
                lock.notifyAll();
            }
        }
    }
        ...
        synchronized (lock)
        {
            ...
            while (itr.hasNext())
            {
                ...
                if (publisher.isTimeout())
                {
                    itr.remove();
                    publisher.close();
                    logger.debug("publisher-{} timeout and close automatically", publisher.publisherId);
                }
            }
            lock.notifyAll();
        }
    ...
    static class Publisher
    {
        ...
        public Publisher(long publisherId,String sim,int lcn)
        {   this.lcn=lcn;
        this.sim=sim;
            this.publisherId = publisherId;
            this.fifoFilePath = new File(new File(Configs.get("fifo-pool.path")), sim+"_"+lcn + ".fifo").getAbsolutePath();
        ...
        public void open(String rtmpURL) throws Exception
        {
            ...
            process = Runtime.getRuntime().exec(String.format("%s -re -i %s -c copy -an -f flv %s", Configs.get("ffmpeg.path"), fifo.getAbsolutePath(), rtmpURL));
        ...
        public void close()
        {
            ...
            try { new File(fifoFilePath).delete(); } catch(Exception e) {
                logger.error("fifoFilePath).delete", e);
            }
            try { fileChannel.close(); } catch(Exception e) {
                logger.error("fileChannel.close()", e);
            }
            byteBuffer = null;
            fileChannel = null;
            process = null;
        ...
        {
            return "Publisher{hash=" + hashCode() + ", " + "publisherId=" + publisherId + ", fifoFilePath='" + fifoFilePath + '\'' + ", lastActiveTime=" + lastActiveTime + '}';
        }
    }
